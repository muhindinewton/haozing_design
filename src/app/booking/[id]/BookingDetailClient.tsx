...
export default function BookingDetailClient({ booking }: { booking: typeof BOOKINGS[0] }) {
      ...
      <div className="max-w-4xl mx-auto px-4 py-4 md:py-8">
        <div className="flex items-center justify-between mb-4 md:mb-6">
          <Link href="/trips" className="flex items-center gap-2 text-stone-600 hover:text-stone-900 transition-colors text-sm">
            <ArrowLeft className="w-4 h-4" /> <span className="hidden sm:inline">My Trips</span>
          </Link>
          <span className={`${statusColors[booking.status]} text-xs md:text-sm`}>{booking.status.charAt(0).toUpperCase() + booking.status.slice(1)}</span>
        </div>

        {/* Hero card */}
        <div className="card overflow-hidden mb-4 md:mb-6">
          <div className="relative h-40 sm:h-48">
            ...
            <div className="absolute bottom-3 md:bottom-4 left-3 md:left-4 text-white">
              <h1 className="font-display text-lg sm:text-xl md:text-2xl font-bold">{booking.propertyTitle}</h1>
              <p className="text-xs md:text-sm text-white/80 flex items-center gap-1 mt-0.5"><MapPin className="w-3 h-3 md:w-3.5 md:h-3.5" />{booking.location}</p>
            </div>
            <div className="absolute top-3 md:top-4 right-3 md:right-4 bg-white/20 backdrop-blur-sm border border-white/30 rounded-lg md:rounded-xl px-2 md:px-3 py-1">
              <span className="text-white text-[10px] md:text-xs font-bold">#{booking.id}</span>
            </div>
          </div>

          {/* Dates bar */}
          <div className="grid grid-cols-3 divide-x divide-stone-100 p-3 md:p-4 text-center md:text-left">
            <div className="px-2 md:px-4 first:pl-0">
              <p className="text-[10px] md:text-xs text-stone-500 uppercase tracking-wide font-semibold mb-0.5 md:mb-1">Check-in</p>
              <p className="font-bold text-stone-900 text-xs md:text-base">{new Date(booking.checkIn).toLocaleDateString('en-KE', { month: 'short', day: 'numeric' })}</p>
              <p className="text-[10px] md:text-xs text-stone-500 hidden sm:block">From 2:00 PM</p>
            </div>
            <div className="px-2 md:px-4">
              <p className="text-[10px] md:text-xs text-stone-500 uppercase tracking-wide font-semibold mb-0.5 md:mb-1">Duration</p>
              <p className="font-bold text-stone-900 text-xs md:text-base">{nights} nights</p>
              <p className="text-[10px] md:text-xs text-stone-500">{booking.guests} guests</p>
            </div>
            <div className="px-2 md:px-4">
              <p className="text-[10px] md:text-xs text-stone-500 uppercase tracking-wide font-semibold mb-0.5 md:mb-1">Check-out</p>
              <p className="font-bold text-stone-900 text-xs md:text-base">{new Date(booking.checkOut).toLocaleDateString('en-KE', { month: 'short', day: 'numeric' })}</p>
              <p className="text-[10px] md:text-xs text-stone-500 hidden sm:block">By 11:00 AM</p>
            </div>
          </div>
        </div>

        {/* Tabs */}
        <div className="flex gap-0.5 md:gap-1 border-b border-stone-200 mb-4 md:mb-6">
          {(['details', 'checkin', 'payment'] as const).map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)} className={`px-3 md:px-5 py-2.5 md:py-3 text-xs md:text-sm font-semibold capitalize transition-all border-b-2 -mb-px ${activeTab === tab ? 'border-brand-700 text-brand-700 bg-white' : 'border-transparent text-stone-500 hover:text-brand-700'}`}> 
          ...
          <div className="grid md:grid-cols-2 gap-4 animate-fade-in">
            ...
            <div className="card p-5">
              ...
              <div className="flex gap-2">
                ...
                <a href={`https://wa.me/${booking.hostPhone.replace(/[^0-9]/g,'')}`} target="_blank" rel="noopener noreferrer" className="flex-1 flex items-center justify-center gap-2 bg-green-50 transition-all rounded-xl py-2.5 text-sm font-semibold border border-green-100 hover:bg-green-100 text-green-700">
          ...
          <div className="space-y-4 animate-fade-in">
            {booking.accessCode ? (
              <div className="card p-6 text-center">
                ...
                <button onClick={copyCode} className={`flex items-center gap-2 mx-auto px-6 py-3 rounded-xl font-semibold transition-all text-sm ${codeCopied ? 'bg-emerald-500 text-white' : 'border border-brand-700 text-brand-700 hover:bg-brand-50'}`}> 
