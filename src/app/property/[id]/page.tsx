    ...
    <div className="min-h-screen bg-surface">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="flex items-center justify-between mb-4">
          <Link href="/search" className="flex items-center gap-2 text-stone-600 hover:text-stone-900 transition-colors">
            <ArrowLeft className="w-4 h-4" /> Back to search
          </Link>
          <div className="flex gap-2">
            <button onClick={() => setLiked(!liked)} className="flex items-center gap-2 px-4 py-2 rounded-xl border border-stone-200 hover:border-stone-300 text-sm font-medium transition-all">
              <Heart className={`w-4 h-4 ${liked ? 'fill-red-500 text-red-500' : 'text-stone-600'}`} /> Save
            </button>
            <button className="flex items-center gap-2 px-4 py-2 rounded-xl border border-stone-200 hover:border-stone-300 text-sm font-medium transition-all">
        ...
        <div className="mb-5">
          <div className="flex flex-wrap items-center gap-2 mb-2">
            {property.verified && <span className="verified-badge"><Shield className="w-3 h-3" /> Verified</span>}
            {property.instantBook && <span className="badge-blue"><Zap className="w-3 h-3" /> Instant Book</span>}
            <span className="badge-gray">{property.type}</span>
          </div>
          <h1 className="font-display text-3xl md:text-4xl font-bold text-stone-900 mb-2">{property.title}</h1>
          <div className="flex flex-wrap items-center gap-4 text-sm text-stone-600">
            <span className="flex items-center gap-1"><MapPin className="w-4 h-4 text-brand-700" />{property.location}</span>
            <span className="flex items-center gap-1"><Star className="w-4 h-4 fill-savanna-500 text-savanna-500" /><strong>{property.rating}</strong> ({property.reviewCount} reviews)</span>
            <span><Bed className="w-4 h-4 inline mr-1" />{property.beds} beds</span>
            <span><Bath className="w-4 h-4 inline mr-1" />{property.baths} baths</span>
            <span><Users className="w-4 h-4 inline mr-1" />{property.guests} guests</span>
          </div>
        </div>

        {/* Image gallery */}
        <div className="grid grid-cols-4 gap-2 rounded-2xl overflow-hidden mb-8 h-80 md:h-96">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-4 card p-4 mb-6">
              <img src={property.hostAvatar} className="w-12 h-12 rounded-full object-cover" alt={property.hostName} />
              <div className="flex-1">
                <p className="font-semibold text-stone-900">Hosted by {property.hostName}</p>
                <p className="text-xs text-stone-500">Superhost · 3 years hosting</p>
              </div>
              <div className="flex gap-2">
                <a href={`tel:${property.hostId}`} className="btn-secondary py-2 px-3 text-sm flex items-center gap-2"><Phone className="w-4 h-4" /> Call</a>
                <Link href="/messages" className="btn-secondary py-2 px-3 text-sm flex items-center gap-2"><MessageCircle className="w-4 h-4" /> Message</Link>
              </div>
            </div>
            ...
            <div className="flex gap-1 border-b border-stone-200 mb-6 overflow-x-auto">
              {(['overview', 'amenities', 'reviews', 'location', 'utilities'] as const).map(tab => (
                <button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-2.5 text-sm font-semibold capitalize whitespace-nowrap transition-all border-b-2 -mb-px ${activeTab === tab ? 'border-brand-700 text-brand-700' : 'border-transparent text-stone-500 hover:text-stone-900'}`}>
                  {tab}
                </button>
              ))}
            </div>
          ...
          <div className="lg:w-96 shrink-0">
            <div className="card p-5 sticky top-20">
              <div className="flex items-baseline gap-2 mb-5">
                <span className="font-display text-3xl font-bold text-stone-900">KSh {property.price.toLocaleString()}</span>
              ...
              <div className="grid grid-cols-2 gap-1 border border-stone-200 rounded-xl overflow-hidden mb-3">
                <div className="p-3 border-r border-stone-200">
                  <label className="text-xs font-bold text-stone-500 uppercase tracking-wide block mb-1">Check-in</label>
                  <input type="date" value={checkIn} onChange={e => setCheckIn(e.target.value)} className="w-full text-sm font-medium text-stone-900 bg-transparent outline-none" />
                </div>
                <div className="p-3">
                  <label className="text-xs font-bold text-stone-500 uppercase tracking-wide block mb-1">Check-out</label>
                  <input type="date" value={checkOut} onChange={e => setCheckOut(e.target.value)} className="w-full text-sm font-medium text-stone-900 bg-transparent outline-none" />
                </div>
              </div>

              <div className="border border-stone-200 rounded-xl p-3 mb-4">
                <label className="text-xs font-bold text-stone-500 uppercase tracking-wide block mb-2">Guests</label>
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium">{guests} guest{guests > 1 ? 's' : ''}</span>
                  <div className="flex items-center gap-2">
                    <button onClick={() => setGuests(Math.max(1, guests - 1))} className="w-7 h-7 rounded-full border border-stone-200 text-stone-700 flex items-center justify-center font-bold hover:border-stone-400 transition-all">−</button>
                    <span className="w-4 text-center font-semibold">{guests}</span>
                    <button onClick={() => setGuests(Math.min(property.guests, guests + 1))} className="w-7 h-7 rounded-full border border-stone-200 text-stone-700 flex items-center justify-center font-bold hover:border-stone-400 transition-all">+</button>
                  </div>
                </div>
              </div>

              <Link href={`/booking/${property.id}`} className="btn-primary w-full block text-center mb-3">Reserve Now</Link>
              <p className="text-xs text-stone-400 text-center mb-4">You won't be charged yet</p>
